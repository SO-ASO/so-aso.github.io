...
function slideRight(name) {
    ...
    {
        slider.style.marginLeft = eval('margin'+name)+ (100 / slidesPerPage-1)+"%";
        eval('margin'+name+'+=100/'+slidesPerPage);
        eval('position'+name+'--');
    } 
    if (eval('position'+name) === 0) {
...
function slideLeft(name) {
    ...
    if(eval('position'+name)!=slides-slidesPerPage)
    {
        slider.style.marginLeft =eval('margin'+name)-120-(100 / slidesPerPage)+"%";
        eval('margin'+name+'-=100/'+slidesPerPage);
        eval('position'+name+'++');
    }
    if(eval('position'+name) == slides-slidesPerPage){
